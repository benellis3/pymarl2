...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        stochastic_attack: bool, optional
            When attacks are stochastic, each agent has a probability p_i that
            their attack is successful. This is private to each agent, but is part
            of the central state and is generated each episode (default is False).
        random_start: bool, optional
            When this is true, the allied units are moved to random positions at the
    ...
    def _generate_attack_probs(self):
        params = self.rng.uniform(
            low=self.attack_probability_min,
            high=self.attack_probability_max,
            size=(self.n_agents,),
        )
        if self.debug:
            logging.debug(f"attack probabilities: {params}")
        return params
    ...
    def reset(self):
        ...
        # reset the attack probabilities:
        self.attack_probabilities = self._generate_attack_probs()
    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
            ...
            if self.stochastic_attack:
                p = self.rng.uniform()
                if p > self.attack_probabilities[a_id]:
                    if self.debug:
                        logging.debug(
                            f"Agent {a_id} {action_name}s unit # {target_id}, but fails"
                        )
                    return None
            cmd = r_pb.ActionRawUnitCommand(
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            if self.stochastic_attack:
                own_feats[ind] = self.attack_probabilities[agent_id]
                ind += 1
    ...
    def get_state(self):
        ...
        nf_al = self._get_ally_feat_size()
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                if self.stochastic_attack:
                    ally_state[al_id, ind] = self.attack_probabilities[al_id]
                    ind += 1
