...
config=$1  # qmix
tag=$2
maps=${3:-sc2_gen_protoss}   # MMM2 left out
units=${8:-5}
offset=1
threads=${4:-9} # 2
td_lambdas=${9:-0.6}
            ...
            for map in "${maps[@]}"; do
                for unit in "${units[@]}"; do
                    for((i=0;i<times;i++)); do
                        ...
                        enemies=$(($unit + $offset))
                        $debug ./run_docker.sh $gpu python3 src/main.py --no-mongo --config="$config" --env-config="$map" with group="$group" env_args.capability_config.n_units=$unit env_args.capability_config.n_enemies=$enemies env_args.prob_obs_enemy=$prob use_wandb=False td_lambda=$tdlambda epsilon_anneal_time=$epsanneal save_model=True "${args[@]}" &
